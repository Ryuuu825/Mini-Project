...
// {
//     extern u16 reg[16];
//     u16 destination_register = (instruction >> 9) & 0b111;
//     u16 offset = sign_extend(instruction & 0x1FF , 9);
...
void store(u16 instruction)
{
    extern u16 reg[16];
    u16 offset = sign_extend(instruction & 0x1FF , 9);
    u16 data = reg[(instruction >> 9) & 0b111];
    mem_write( reg[R_PC] + offset , data);
}
...
void store_indirect(u16 instruction)
{
    ...
    u16 offset = sign_extend(instruction & 0x1FF , 9); 
    u16 data = reg[(instruction >> 9) & 0b111];
    mem_write(mem_read(reg[R_PC] + offset) , data);
}

void store_register(u16 instruction)
{
    extern u16 reg[16];
    u16 base_register = (instruction >> 6) & 0b111;
    u16 offset = (instruction) & 0b11111;
    u16 data = reg[(instruction >> 9) & 0b111];
    mem_write( reg[base_register] + offset , data);
}
